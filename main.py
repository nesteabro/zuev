...
class VariantData:
    ...
    task1_u3 = 170.0       # Напряжение зажигания U3, В (из таблицы)

    # Практическая работа №2
    task2_lambda_min = 4.4 # Длина волны, умноженная на 10^-11 м (из таблицы)

    # Практическая работа №3
    task3_p = 3.4          # Мощность лазера P, мВт (из таблицы)

    # Практическая работа №4
    task4_lambda = 0.536    # Длина волны падающего света, мкм (из таблицы)

    # Практическая работа №5
    task5_u = 4.4          # Напряжение U, В (из таблицы)

    # Практическая работа №6 (Для этой задачи нет таблицы вариантов, задаем угол вручную)
    task6_theta_deg = 90.0 # Угол рассеяния в градусах

    # Практическая работа №8
    task8_i0 = 9.3         # Интенсивность пучка I0, Вт/м^2 (из таблицы)
    task8_r = 0.74         # Коэффициент отражения R (из таблицы)

    task9_alpha = 6.4      # Угол при вершине призмы, град
    task9_n = 1.64         # Показатель преломления
    task10_i = 114.0       # Мощность источника, мВт
    task10_x = 7.4         # Расстояние x, мм
    task10_d = 11.4        # Диаметр фотоприемника d, мм
    task11_phi = 114.0     # Поток излучения, мВт
    task12_i0 = 9.4        # Интенсивность пучка I0, Вт/м^2 
    task12_r = 0.73        # Коэффициент отражения R 
    
    task13_lambda = 1.8    # Длина волны, мкм
    task14_n = 16.0        # Во сколько раз больше пропускает щель В
    task15_x = 16.0        # Толщина медной пластинки x, см
    task16_alpha = 4.84    # Коэффициент отражения alpha (в % или долях)
    task16_N = 4           # Количество пластин N

# ==========================================
# РЕШЕНИЯ ЗАДАЧ С ВЫВОДОМ ШАГОВ
# ==========================================
...
def solve_task_4(lambda_um):
    ...
    v = math.sqrt((2 * H * C * (lambda_max_m - lambda_m)) / (M_E * lambda_m * lambda_max_m))
